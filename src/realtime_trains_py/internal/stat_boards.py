...
class NewStationBoard:
    ...
    def __init__(self, rows=None, departure_data=None, arrival_data=None) -> None:
        ...
        for dep_service in departure_data["services"]:
            departure_board.append(create_dep_service(dep_service))
            count +=1 
            if count == rows:
                break
        ...
        for arr_service in arrival_data["services"]:
            arrival_board.append(create_arr_service(arr_service))
            count +=1 
            if count == rows:
                break
...
def create_dep_service(service) -> tuple:
...
def create_arr_service(service) -> tuple:
